...
export default function Messenger({ data }) {
  ...
  useEffect(() => {
    ...
    socket.current.on("getUsers", (users) => {
      console.log(
        "users from server side to client:",
        users[0].userid || users[1]
      );
    });
  }, [tokenDetails]);
  ...
  const getConversation = async () => {
    try {
      let res = await axios.get(
        `http://localhost:3000/api/conversation/${tokenDetails._id}`
      );
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    const receivedId = currentChat.members.find(
      (member) => member !== tokenDetails._id
    );
            ...
            <Box w="45%" className={styles.chatBox}>
              ...
              <Flex>
                <Input
                  ...
                  value={newMessage}
                />
                <Button bg={"teal"} color={"white"} onClick={handleSubmit}>
                  Send
                </Button>
              </Flex>
            </Box>
